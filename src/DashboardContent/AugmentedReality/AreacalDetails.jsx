...
const AreacalDetails = () => {
  ...
  const translations = {
    ...
    uploadNewImageButton: language === 'th' ? 'อัปโหลดรูปใหม่' : 'Upload New Image',
    noImagesMessage: language === 'th' ? 'ไม่มีรูปภาพสำหรับพื้นที่นี้' : 'No images available for this area',
    ...
    uploadImageError: language === 'th' ? 'ไม่สามารถอัปโหลดรูปได้' : 'Unable to upload image',
    deleteImageSuccess: language === 'th' ? 'ลบสำเร็จ!' : 'Deleted successfully!',
    deleteImageError: language === 'th' ? 'ไม่สามารถลบรูปได้' : 'Unable to delete image',
    updateImageSuccess: language === 'th' ? 'อัปเดตรูปสำเร็จ!' : 'Update Image',
    updateImageError: language === 'th' ? 'ไม่สามารถอัปเดตรูปได้' : 'Unable to update image',
    uploadImageTitle: language === 'th' ? 'อัปโหลดรูปภาพ' : 'Upload Image',
    chooseFileMessage: language === 'th' ? 'กรุณาเลือกไฟล์ก่อนอัปโหลด!' : 'Please select a file to upload!',
    confirmDeleteMessage: language === 'th' ? 'รูปภาพนี้จะถูกลบถาวร!' : 'This image will be permanently deleted!',
    confirmDeleteTitle: language === 'th' ? 'คุณแน่ใจหรือไม่?' : 'Are you sure?',
  ...
  const updateImagePopup = (id) => {
    Swal.fire({
      ...
      preConfirm: async () => {
        ...
        try {
          await axios.delete(`${import.meta.env.VITE_SERVER_URL}/area_images/${id}`); // Delete old image
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 font-prompt">
      ...
      {loading ? (
        ...
        <p className="text-center text-gray-500 text-lg">{translations.noImagesMessage}</p>
      ) : (
            ...
            >
              ...
              <div className="p-4 bg-gray-50 flex flex-col items-center">
                <p className="text-sm text-gray-500 text-center">
                  📅 {translations.uploadedAt}: {new Date(image.uploaded_at).toLocaleDateString()}
                </p>
