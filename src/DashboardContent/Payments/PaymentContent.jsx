...
const PaymentContent = () => {
  ...
  const handleDelete = async (paymentId) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await axios.delete(`${apiUrl}/payments/${paymentId}`);
    ...
    <div className="p-8 rounded-lg shadow-lg w-full mx-auto font-inter h-full">
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to="/dashboard/payments/add">
          <button className="btn bg-blue text-white hover:bg-blue">Add Payment</button>
        </Link>
      </div>
          ...
          {payments.length > 0 ? (
            payments.map((payment) => (
              <tr key={payment.payment_id}>
                ...
                <td className="border border-gray-300 p-2">{payment.payment_method}</td>
                <td className="border border-gray-300 p-2">{new Date(payment.payment_date).toLocaleString()}</td>
                <td className="border border-gray-300 p-2">
                  {payment.slip_images ? (
                    <img
                      src={`${apiUrl}${payment.slip_images}`}
                      alt="Slip"
            ...
            <tr>
              <td colSpan="10" className="border border-gray-300 p-4">No payments available</td>
            </tr>
          )}
        </tbody>
      </table>
