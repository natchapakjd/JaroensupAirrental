...
const EditPayment = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [paymentResponse, usersResponse, tasksResponse, ordersResponse] = await Promise.all([
        ...
        // Assuming payment_date is in 'YYYY-MM-DD HH:mm:ss' format
        setPaymentDate(paymentData.payment_date ? paymentData.payment_date.substring(0, 16) : ''); // Format for datetime-local
  ...
  return (
    <div className="p-8 font-inter">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className="btn bg-blue text-white">Update Payment</button>
      </form>
    </div>
  );
};
