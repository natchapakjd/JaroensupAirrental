    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        if (response.status === 200) {
          Swal.fire(translations[language].deleteSuccessTitle, translations[language].deleteSuccessText, 'success');
    ...
    <div className="p-8 rounded-lg shadow-lg w-full mx-auto h-screen">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/dashboard/attributes/add">
          <button className="btn bg-blue text-white hover:bg-blue">{translations[language].addAttribute}</button>
        </Link>
      </div>
        ...
        <div className="overflow-x-auto"><table className="table w-full border-collapse border border-gray-300 text-center">
            <thead className="sticky-top bg-gray-200">
              <tr>
                ...
                <th className="border p-2 text-center">{translations[language].name}</th>
