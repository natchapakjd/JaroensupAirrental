...
const OrderDetails = () => {
  ...
  const translations = {
    ...
    en: {
      ...
      errorLoading: 'Unable to load order details.',
      noDetailsAvailable: 'No order details available.',
    },
  };
  ...
  return (
    <div className="p-8 rounded-lg shadow-lg w-full mx-auto font-prompt h-full">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold mb-4">{translations[language].orderDetailsTitle}</h2>
      ...
      {order ? (
        <div>
          <div className="mb-4">
            <h3 className="font-bold">{translations[language].orderId} {order.order_id}</h3>
            <p>{translations[language].user} {order.firstname} {order.lastname}</p>
            <p>{translations[language].orderDate} {new Date(order.created_at).toLocaleString()}</p>
          </div>
          <h4 className="font-semibold mb-2">{translations[language].itemsTitle}</h4>
