    ...
    <div className="p-8 bg-gray-50 min-h-screen font-prompt">
      <div className="bg-white p-6 rounded-lg shadow-xl mb-6">
        <h1 className="text-4xl font-bold text-gray-800">
          {t.welcomeMessage.replace("{name}", user ? `${user.firstname} ${user.lastname}` : 'User')}
        </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-gradient-to-r from-red-500 to-pink-600 text-white p-6 rounded-lg shadow-lg flex items-center justify-between transition-all hover:scale-105 hover:shadow-2xl">
          <div>
            <h2 className="text-xl font-semibold">{t.totalRevenue}</h2>
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg mb-6">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">{t.quickLinks}</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Link to="/dashboard/orders" className="bg-green-600 text-white p-4 rounded-lg shadow-lg text-center transition-all hover:bg-green-700 hover:shadow-xl">
            <h3 className="text-xl font-semibold">{t.viewOrders}</h3>
            <p className="mt-2">{t.seeAllRecentOrders}</p>
          </Link>
          <Link to="/dashboard/analytics" className="bg-yellow-500 text-white p-4 rounded-lg shadow-lg text-center transition-all hover:bg-yellow-600 hover:shadow-xl">
            <h3 className="text-xl font-semibold">{t.viewAnalytics}</h3>
            <p className="mt-2">{t.seeAllAnalysis}</p>
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">{t.recentActivity}</h2>
        <ul className="divide-y divide-gray-200">
          {recentActivity.length > 0 ? (
            recentActivity.map((activity, index) => (
              <li key={index} className="py-4 flex items-center gap-4 hover:bg-gray-50 rounded-lg p-3 transition">
                ...
                <div>
                  <p className="font-semibold text-gray-700 text-lg">{activity.description}</p>
                  <p className="text-sm text-gray-500">
                    {t.taskType}: {activity.task_type_id}
                    <span className="text-blue-600 font-medium">{activity.type_name}</span> | {t.status}: 
                    <span
            ...
            <li className="text-gray-500 text-center py-4">{t.noRecentActivity}</li>
          )}
        </ul>
      </div>
    </div>
